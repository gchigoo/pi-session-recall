...
export function searchChunks(
  ...
  // project 硬过滤必须在 FTS LIMIT 之前，避免他项目高分挤掉本项目候选。
  // sessions.status=active 等可变条件放在 LIMIT 之后 JOIN，避免对全库做 IN 物化拖慢热路径。
  const ftsFilters: string[] = ["chunks_fts MATCH ?"];
  const ftsParams: Array<string | number> = [parsed.matchExpression];
  if (options.scope === "project") {
    const projectKey = options.projectKey;
    if (!projectKey) {
      throw new Error(ERROR_CODES.QUERY_INVALID);
    }
    ftsFilters.push(`source_key IN (SELECT source_key FROM chunks WHERE origin_project_key = ?)`);
    ftsParams.push(projectKey);
  }

  const postFilters: string[] = ["s.status = 'active'"];
  const postParams: Array<string | number> = [];
  if (options.excludeSessionId) {
    postFilters.push("c.session_id != ?");
    postParams.push(options.excludeSessionId);
  }
  if (options.autoEligibleOnly) {
    postFilters.push("c.auto_eligible = 1");
  }
  if (options.verifiedOnly) {
    postFilters.push("c.provenance = 'verified'");
  }
